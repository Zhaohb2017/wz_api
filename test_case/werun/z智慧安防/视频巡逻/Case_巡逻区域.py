...
from business.param_config.api_param.werun.基础管理 import Param_登录登出, Param_设备类型, Param_空间信息, Param_空间类型, Param_楼栋信息, \
    Param_项目管理
from business.param_config.api_param.werun.智慧安防.视频巡逻 import Param_巡逻区域, Param_巡逻标准
...
@ddt.ddt
class Case巡逻区域(unittest.TestCase):
    def setUp(self):
        # 获取requests.session
    ...
    def test_巡逻区域_01(self):
        """
        单接口: 1/新增巡逻区域
        :return:
        """

        #新增巡逻区域(依赖项目、巡逻标准、摄相头)
        feild_xlqy = Param_巡逻区域.p_add.get('01')
        res_data = Page巡逻区域配置表管理.api_新增巡逻区域配置(self.session,feild_xlqy,self.header)

    def test_巡逻区域_02(self):
        """
        单接口: 2/id查询
        :return:
        """
        # 通过id查询巡逻区域，并断言
        res_data = Page巡逻区域配置表管理.api_巡逻区域配置通过id查询(self.session, str(12313), self.header)

    def test_巡逻区域_03(self):
        """
        单接口: 3/id删除
        :return:
        """
        #通过id删除巡逻区域
        res_data = Page巡逻区域配置表管理.api_巡逻区域配置通过id删除(self.session,str(65456),self.header)

    def test_巡逻区域_04(self):
        """
        单接口: 4/分页查询
        :return:
        """
        feild_page = Param_巡逻区域.p_page.get('名称')
        res_data = Page巡逻区域配置表管理.api_巡逻区域配置分页查询(self.session,feild_page,self.header)

    def test_巡逻区域_05(self):
        """
        单接口: 5/启用禁用
        :return:
        """
        ...
        #禁用巡逻区域
        feild_off = Param_巡逻区域.p_on_off.get('禁用')
        feild_off['id'] = 12312
        res_data = Page巡逻区域配置表管理.api_巡逻区域配置启用禁用(self.session,feild_off,self.header)

    def test_巡逻区域_06(self):
        """
        单接口: 6/编辑
        :return:
        """
        #编辑修改巡逻区域
        feild_upd = Param_巡逻区域.p_upd.get('01')
        res_data = Page巡逻区域配置表管理.api_修改巡逻区域配置(self.session,feild_upd,self.header)
