...
class Page巡检点下设备与位置表管理(object):

    @classmethod
    def api_新增巡检点下设备及位置表(cls, session: requests.Session, feilds: dict,header:dict):
        api_name = 'api_新增巡检点下设备及位置表'
    ...
    @classmethod
    def api_修改巡检点下设备及位置表(cls, session: requests.Session, feilds: dict,header:dict):
        api_name = 'api_修改巡检点下设备及位置表'
        url = Api_Addr_WeRun.werun_apis.get(api_name)
        header.update(Browser_Param.headers.get('json').copy())
    ...
    @classmethod
    def api_巡检点下设备及位置通过id查询(cls, session: requests.Session, id: str,header:dict):
        api_name = 'api_巡检点下设备及位置通过id查询'
    ...
    @classmethod
    def api_巡检点下设备及位置通过id删除(cls, session: requests.Session, id: str,header:dict):
        api_name = 'api_巡检点下设备及位置通过id删除'
        url = Api_Addr_WeRun.werun_apis.get(api_name)+id
        header.update(Browser_Param.headers.get('form').copy())
