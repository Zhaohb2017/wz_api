...
class Page巡检点表管理(object):

    @classmethod
    def api_新增巡检点表(cls, session: requests.Session, feilds: dict,header:dict):
        api_name = 'api_新增巡检点表'
    ...
    @classmethod
    def api_修改巡检点表(cls, session: requests.Session, feilds: dict,header:dict):
        api_name = 'api_修改巡检点表'
        url = Api_Addr_WeRun.werun_apis.get(api_name)
        header.update(Browser_Param.headers.get('json').copy())
    ...
    @classmethod
    def api_获取巡检点的设备及空间分页列表(cls, session: requests.Session, feilds: dict,header:dict):
        api_name = 'api_获取巡检点的设备及空间分页列表'
        url = Api_Addr_WeRun.werun_apis.get(api_name)
        header.update(Browser_Param.headers.get('json').copy())
        r = session.post(url, headers=header, json=feilds)
        Common_Base.logging_api_info(api_name, url, str(header) ,r.text, str(feilds),request_method=r.request.method)
    ...
    @classmethod
    def api_查询巡检点分页列表(cls, session: requests.Session, feilds: dict,header:dict):
        api_name = 'api_查询巡检点分页列表'
        url = Api_Addr_WeRun.werun_apis.get(api_name)
        header.update(Browser_Param.headers.get('json').copy())
        r = session.post(url, headers=header, json=feilds)
        try:
    ...
    @classmethod
    def api_查询巡检点及内容记录列表(cls, session: requests.Session, feilds: dict,header:dict):
        api_name = 'api_查询巡检点及内容记录列表'
        url = Api_Addr_WeRun.werun_apis.get(api_name)
    ...
    @classmethod
    def api_巡检点通过id查询(cls, session: requests.Session, id: str,header:dict):
        api_name = 'api_巡检点通过id查询'
    ...
    @classmethod
    def api_巡检点通过id删除(cls, session: requests.Session, id: str,header:dict):
        api_name = 'api_巡检点通过id删除'
        url = Api_Addr_WeRun.werun_apis.get(api_name)+id
        header.update(Browser_Param.headers.get('form').copy())
