...
class Page巡检记录的状态为完成时参数值表管理(object):

    @classmethod
    def api_新增巡检记录的状态为完成时参数值表(cls, session: requests.Session, feilds: dict,header:dict):
        api_name = 'api_新增巡检记录的状态为完成时参数值表'
    ...
    @classmethod
    def api_修改巡检记录的状态为完成时参数值表(cls, session: requests.Session, feilds: dict,header:dict):
        api_name = 'api_修改巡检记录的状态为完成时参数值表'
        url = Api_Addr_WeRun.werun_apis.get(api_name)
        header.update(Browser_Param.headers.get('json').copy())
    ...
    @classmethod
    def api_巡检记录完成状态参数值通过id查询(cls, session: requests.Session, id: str,header:dict):
        api_name = 'api_巡检记录完成状态参数值通过id查询'
    ...
    @classmethod
    def api_巡检记录完成状态参数值通过id删除(cls, session: requests.Session, id: str,header:dict):
        api_name = 'api_巡检记录完成状态参数值通过id删除'
        url = Api_Addr_WeRun.werun_apis.get(api_name)+id
        header.update(Browser_Param.headers.get('form').copy())
