...
class Page巡计划与巡检点关联表管理(object):

    @classmethod
    def api_新增巡计划与巡检点关联表(cls, session: requests.Session, feilds: dict,header:dict):
        api_name = 'api_新增巡计划与巡检点关联表'
    ...
    @classmethod
    def api_修改巡计划与巡检点关联表(cls, session: requests.Session, feilds: dict,header:dict):
        api_name = 'api_修改巡计划与巡检点关联表'
        url = Api_Addr_WeRun.werun_apis.get(api_name)
        header.update(Browser_Param.headers.get('json').copy())
    ...
    @classmethod
    def api_巡计划与巡检点关联通过id查询(cls, session: requests.Session, id: str,header:dict):
        api_name = 'api_巡计划与巡检点关联通过id查询'
    ...
    @classmethod
    def api_巡计划与巡检点关联通过id删除(cls, session: requests.Session, id: str,header:dict):
        api_name = 'api_巡计划与巡检点关联通过id删除'
        url = Api_Addr_WeRun.werun_apis.get(api_name)+id
        header.update(Browser_Param.headers.get('form').copy())
