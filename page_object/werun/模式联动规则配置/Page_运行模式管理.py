...
class Page运行模式管理(object):
    ...
    @classmethod
    def api_修改运行模式(cls, session: requests.Session, feilds: dict,header:dict):
        api_name = 'api_修改运行模式'
        url = Api_Addr_WeRun.werun_apis.get(api_name)
        header.update(Browser_Param.headers.get('json').copy())
    ...
    @classmethod
    def api_查询设备运行模式列表(cls, session: requests.Session, feilds: dict,header:dict):
        api_name = 'api_查询设备运行模式列表'
        url = Api_Addr_WeRun.werun_apis.get(api_name)
        header.update(Browser_Param.headers.get('json').copy())
        r = session.post(url, headers=header, json=feilds)
        try:
    ...
    @classmethod
    def api_运行模式集合查询(cls, session: requests.Session, feilds: dict,header:dict):
        api_name = 'api_运行模式集合查询'
        url = Api_Addr_WeRun.werun_apis.get(api_name)
        header.update(Browser_Param.headers.get('json').copy())
        r = session.post(url, headers=header, json=feilds)
        try:
    ...
    @classmethod
    def api_运行模式分页查询(cls, session: requests.Session, feilds: dict,header:dict):
        api_name = 'api_运行模式分页查询'
        url = Api_Addr_WeRun.werun_apis.get(api_name)
        header.update(Browser_Param.headers.get('json').copy())
        r = session.post(url, headers=header, json=feilds)
        try:
    ...
    @classmethod
    def api_运行模式启用禁用(cls, session: requests.Session, feilds: dict,header:dict):
        api_name = 'api_运行模式启用禁用'
        url = Api_Addr_WeRun.werun_apis.get(api_name)
        header.update(Browser_Param.headers.get('json').copy())
        r = session.post(url, headers=header, json=feilds)
        try:
    ...
    @classmethod
    def api_运行模式通过id查询(cls, session: requests.Session, id: str,header:dict):
    ...
    @classmethod
    def api_运行模式通过id删除(cls, session: requests.Session, id: str,header:dict):
        api_name = 'api_运行模式通过id删除'
        url = Api_Addr_WeRun.werun_apis.get(api_name)+id
        header.update(Browser_Param.headers.get('form').copy())
