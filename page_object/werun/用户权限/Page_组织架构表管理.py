...
class Page组织架构表管理(object):

    @classmethod
    def api_新增组织架构(cls, session: requests.Session, feilds: dict,header:dict):
        api_name = 'api_新增组织架构'
    ...
    @classmethod
    def api_组织架构通过id删除(cls, session: requests.Session, id: str,header:dict):
        api_name = 'api_组织架构通过id删除'
    ...
    @classmethod
    def api_查询上一级组织架构下所有的成员列表(cls, session: requests.Session, feilds: dict,header:dict):
        api_name = 'api_查询上一级组织架构下所有的成员列表'
        url = Api_Addr_WeRun.werun_apis.get(api_name)
        header.update(Browser_Param.headers.get('form').copy())
    ...
    @classmethod
    def api_通过IdOrgCode查询组织架构(cls, session: requests.Session, feilds: dict,header:dict):
        api_name = 'api_通过IdOrgCode查询组织架构'
        url = Api_Addr_WeRun.werun_apis.get(api_name)
        header.update(Browser_Param.headers.get('form').copy())
        r = session.get(url, headers=header, params=feilds)
        Common_Base.logging_api_info(api_name, url, str(header) ,r.text, str(feilds),request_method=r.request.method)
    ...
    @classmethod
    def api_组织架构集合查询(cls, session: requests.Session, feilds: dict,header:dict):
        api_name = 'api_组织架构集合查询'
        url = Api_Addr_WeRun.werun_apis.get(api_name)
        header.update(Browser_Param.headers.get('json').copy())
        r = session.post(url, headers=header, json=feilds)
        Common_Base.logging_api_info(api_name, url, str(header) ,r.text, str(feilds),request_method=r.request.method)
    ...
    @classmethod
    def api_组织架构分页查询(cls, session: requests.Session, feilds: dict,header:dict):
        api_name = 'api_组织架构分页查询'
        url = Api_Addr_WeRun.werun_apis.get(api_name)
        header.update(Browser_Param.headers.get('json').copy())
        r = session.post(url, headers=header, json=feilds)
        Common_Base.logging_api_info(api_name, url, str(header) ,r.text, str(feilds),request_method=r.request.method)
    ...
    @classmethod
    def api_修改组织架构(cls, session: requests.Session, feilds: dict,header:dict):
        api_name = 'api_修改组织架构'
        url = Api_Addr_WeRun.werun_apis.get(api_name)
        header.update(Browser_Param.headers.get('json').copy())
