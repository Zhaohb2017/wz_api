...
class Page角色表管理(object):

    @classmethod
    def api_新增角色(cls, session: requests.Session, feilds: dict,header:dict):
        api_name = 'api_新增角色'
    ...
    @classmethod
    def api_修改角色(cls, session: requests.Session, feilds: dict,header:dict):
        api_name = 'api_修改角色'
        url = Api_Addr_WeRun.werun_apis.get(api_name)
        header.update(Browser_Param.headers.get('json').copy())
    ...
    @classmethod
    def api_角色集合查询(cls, session: requests.Session, feilds: dict,header:dict):
        api_name = 'api_角色集合查询'
        url = Api_Addr_WeRun.werun_apis.get(api_name)
        header.update(Browser_Param.headers.get('json').copy())
        r = session.post(url, headers=header, json=feilds)
    ...
    @classmethod
    def api_角色分页查询(cls, session: requests.Session, feilds: dict,header:dict):
        api_name = 'api_角色分页查询'
        url = Api_Addr_WeRun.werun_apis.get(api_name)
        header.update(Browser_Param.headers.get('json').copy())
        r = session.post(url, headers=header, json=feilds)
    ...
    @classmethod
    def api_角色通过id查询(cls, session: requests.Session, id: str,header:dict):
        api_name = 'api_角色通过id查询'
    ...
    @classmethod
    def api_角色通过id删除(cls, session: requests.Session, id: str,header:dict):
        api_name = 'api_角色通过id删除'
        url = Api_Addr_WeRun.werun_apis.get(api_name)+id
        header.update(Browser_Param.headers.get('form').copy())
