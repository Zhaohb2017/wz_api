...
class Page区域或楼栋表管理(object):

    @classmethod
    def api_新增区域或楼栋表(cls, session: requests.Session, feilds: dict,header:dict):
        api_name = 'api_新增区域或楼栋表'
    ...
    @classmethod
    def api_修改区域或楼栋表(cls, session: requests.Session, feilds: dict,header:dict):
        api_name = 'api_修改区域或楼栋表'
        url = Api_Addr_WeRun.werun_apis.get(api_name)
        header.update(Browser_Param.headers.get('json').copy())
    ...
    @classmethod
    def api_查询楼层树形菜单(cls, session: requests.Session, feilds: dict,header:dict):
        api_name = 'api_查询楼层树形菜单'
        url = Api_Addr_WeRun.werun_apis.get(api_name)
        header.update(Browser_Param.headers.get('json').copy())
        r = session.post(url, headers=header, json=feilds)
        Common_Base.logging_api_info(api_name, url, str(header) ,r.text, str(feilds),request_method=r.request.method)
    ...
    @classmethod
    def api_楼栋信息分页查询(cls, session: requests.Session, feilds: dict,header:dict):
        api_name = 'api_楼栋信息分页查询'
        url = Api_Addr_WeRun.werun_apis.get(api_name)
        header.update(Browser_Param.headers.get('json').copy())
        r = session.post(url, headers=header, json=feilds)
        try:
    ...
    @classmethod
    def api_查询区域楼栋下拉列表(cls, session: requests.Session, feilds: dict,header:dict):
        api_name = 'api_查询区域楼栋下拉列表'
        url = Api_Addr_WeRun.werun_apis.get(api_name)
        header.update(Browser_Param.headers.get('json').copy())
        r = session.post(url, headers=header, json=feilds)
        Common_Base.logging_api_info(api_name, url, str(header), r.text, str(feilds))
    ...
    @classmethod
    def api_区域或楼栋通过id查询(cls, session: requests.Session, id: str,header:dict):
        api_name = 'api_区域或楼栋通过id查询'
    ...
    @classmethod
    def api_区域或楼栋通过id删除(cls, session: requests.Session, id: str,header:dict):
        api_name = 'api_区域或楼栋通过id删除'
        url = Api_Addr_WeRun.werun_apis.get(api_name)+id
        header.update(Browser_Param.headers.get('form').copy())
