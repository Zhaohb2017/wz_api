...
class Page省管理(object):

    @classmethod
    def api_新增省区(cls, session: requests.Session, feilds: dict,header:dict):
        api_name = 'api_新增省区'
    ...
    @classmethod
    def api_修改省区(cls, session: requests.Session, feilds: dict,header:dict):
        api_name = 'api_修改省区'
        url = Api_Addr_WeRun.werun_apis.get(api_name)
        header.update(Browser_Param.headers.get('json').copy())
    ...
    @classmethod
    def api_省区集合查询(cls, session: requests.Session, feilds: dict,header:dict):
        api_name = 'api_省区集合查询'
        url = Api_Addr_WeRun.werun_apis.get(api_name)
        header.update(Browser_Param.headers.get('json').copy())
        r = session.post(url, headers=header, json=feilds)
    ...
    @classmethod
    def api_省区分页查询(cls, session: requests.Session, feilds: dict,header:dict):
        api_name = 'api_省区分页查询'
        url = Api_Addr_WeRun.werun_apis.get(api_name)
        header.update(Browser_Param.headers.get('json').copy())
        r = session.post(url, headers=header, json=feilds)
    ...
    @classmethod
    def api_省区通过id查询(cls, session: requests.Session, id: str,header:dict):
        api_name = 'api_省区通过id查询'
    ...
    @classmethod
    def api_省区通过id删除(cls, session: requests.Session, id: str,header:dict):
        api_name = 'api_省区通过id删除'
        url = Api_Addr_WeRun.werun_apis.get(api_name)+id
        header.update(Browser_Param.headers.get('form').copy())
