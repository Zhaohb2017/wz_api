...
class Page巡逻计划与巡逻区域关联表管理(object):

    @classmethod
    def api_新增巡逻计划与巡逻区域关联(cls, session: requests.Session, feilds: dict,header:dict):
        api_name = 'api_新增巡逻计划与巡逻区域关联'
    ...
    @classmethod
    def api_修改巡逻计划与巡逻区域关联(cls, session: requests.Session, feilds: dict,header:dict):
        api_name = 'api_修改巡逻计划与巡逻区域关联'
        url = Api_Addr_WeRun.werun_apis.get(api_name)
        header.update(Browser_Param.headers.get('json').copy())
    ...
    @classmethod
    def api_巡逻计划与巡逻区域关联集合查询(cls, session: requests.Session, feilds: dict,header:dict):
        api_name = 'api_巡逻计划与巡逻区域关联集合查询'
        url = Api_Addr_WeRun.werun_apis.get(api_name)
        header.update(Browser_Param.headers.get('json').copy())
        r = session.post(url, headers=header, json=feilds)
        Common_Base.logging_api_info(api_name, url, str(header) ,r.text, str(feilds),request_method=r.request.method)
    ...
    @classmethod
    def api_巡逻计划与巡逻区域关联分页查询(cls, session: requests.Session, feilds: dict,header:dict):
        api_name = 'api_巡逻计划与巡逻区域关联分页查询'
        url = Api_Addr_WeRun.werun_apis.get(api_name)
        header.update(Browser_Param.headers.get('json').copy())
        r = session.post(url, headers=header, json=feilds)
    ...
    @classmethod
    def api_巡逻计划与巡逻区域关联通过id查询(cls, session: requests.Session, id: str,header:dict):
        api_name = 'api_巡逻计划与巡逻区域关联通过id查询'
    ...
    @classmethod
    def api_巡逻计划与巡逻区域关联通过id删除(cls, session: requests.Session, id: str,header:dict):
        api_name = 'api_巡逻计划与巡逻区域关联通过id删除'
        url = Api_Addr_WeRun.werun_apis.get(api_name)+id
        header.update(Browser_Param.headers.get('form').copy())
