...
class Page空间时态(object):
    ...
    @classmethod
    def api_空间时态_左侧类型(cls, session: requests.Session, feilds: dict,header:dict):
    ...
    @classmethod
    def api_所有楼栋数据(cls, session: requests.Session, feilds: dict,header:dict):
    ...
    @classmethod
    def api_楼层空间信息(cls, session: requests.Session, feilds: dict,header:dict):
        api_name = 'api_楼层空间信息'
        url = Api_Addr_WeRun.werun_apis.get(api_name)
        header.update(Browser_Param.headers.get('json').copy())
        r = session.post(url, headers=header, json=feilds)
    ...
    @classmethod
    def api_楼层房间统计(cls, session: requests.Session, feilds: dict,header:dict):
        api_name = 'api_楼层房间统计'
        url = Api_Addr_WeRun.werun_apis.get(api_name)
        header.update(Browser_Param.headers.get('json').copy())
    ...
    @classmethod
    def api_用户关联所有项目(cls, session: requests.Session,header:dict):
        api_name = 'api_用户关联所有项目'
        url = Api_Addr_WeRun.werun_apis.get(api_name)
        header.update(Browser_Param.headers.get('json').copy())
