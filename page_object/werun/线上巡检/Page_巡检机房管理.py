...
class Page巡检机房管理(object):
    ...
    @classmethod
    def api_修改巡检机房(cls, session: requests.Session, feilds: dict,header:dict):
        api_name = 'api_修改巡检机房'
        url = Api_Addr_WeRun.werun_apis.get(api_name)
        header.update(Browser_Param.headers.get('json').copy())
    ...
    @classmethod
    def api_巡检机房集合查询(cls, session: requests.Session, feilds: dict,header:dict):
        api_name = 'api_巡检机房集合查询'
        url = Api_Addr_WeRun.werun_apis.get(api_name)
        header.update(Browser_Param.headers.get('json').copy())
        r = session.post(url, headers=header, json=feilds)
        Common_Base.logging_api_info(api_name, url, str(header) ,r.text, str(feilds),request_method=r.request.method)
    ...
    @classmethod
    def api_根据楼栋或楼层id查询巡检机房列表(cls, session: requests.Session, feilds: dict,header:dict):
    ...
    @classmethod
    def api_巡检机房分页查询(cls, session: requests.Session, feilds: dict,header:dict):
        api_name = 'api_巡检机房分页查询'
        url = Api_Addr_WeRun.werun_apis.get(api_name)
        header.update(Browser_Param.headers.get('json').copy())
        r = session.post(url, headers=header, json=feilds)
        try:
    ...
    @classmethod
    def api_启用禁用巡检机房(cls, session: requests.Session, feilds: dict,header:dict):
        api_name = 'api_启用禁用巡检机房'
        url = Api_Addr_WeRun.werun_apis.get(api_name)
        header.update(Browser_Param.headers.get('json').copy())
        r = session.put(url, headers=header, json=feilds)
        try:
    ...
    @classmethod
    def api_通过id查询巡检机房(cls, session: requests.Session, id: str,header:dict):
        api_name = 'api_通过id查询巡检机房'
    ...
    @classmethod
    def api_通过id删除巡检机房(cls, session: requests.Session, id: str,header:dict):
        api_name = 'api_通过id删除巡检机房'
        url = Api_Addr_WeRun.werun_apis.get(api_name)+id
        header.update(Browser_Param.headers.get('form').copy())
