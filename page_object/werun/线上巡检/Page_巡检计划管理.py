...
class Page巡检计划管理(object):
    ...
    @classmethod
    def api_修改巡检计划(cls, session: requests.Session, feilds: dict,header:dict):
        api_name = 'api_修改巡检计划'
        url = Api_Addr_WeRun.werun_apis.get(api_name)
        header.update(Browser_Param.headers.get('json').copy())
    ...
    @classmethod
    def api_巡检计划集合查询(cls, session: requests.Session, feilds: dict,header:dict):
        api_name = 'api_巡检计划集合查询'
        url = Api_Addr_WeRun.werun_apis.get(api_name)
        header.update(Browser_Param.headers.get('json').copy())
        r = session.post(url, headers=header, json=feilds)
        Common_Base.logging_api_info(api_name, url, str(header) ,r.text, str(feilds),request_method=r.request.method)
    ...
    @classmethod
    def api_巡检计划分页查询(cls, session: requests.Session, feilds: dict,header:dict):
        api_name = 'api_巡检计划分页查询'
        url = Api_Addr_WeRun.werun_apis.get(api_name)
        header.update(Browser_Param.headers.get('json').copy())
        r = session.post(url, headers=header, json=feilds)
        try:
    ...
    @classmethod
    def api_启用禁用巡检计划(cls, session: requests.Session, feilds: dict,header:dict):
        api_name = 'api_启用禁用巡检计划'
        url = Api_Addr_WeRun.werun_apis.get(api_name)
        header.update(Browser_Param.headers.get('json').copy())
        r = session.put(url, headers=header, json=feilds)
        Common_Base.logging_api_info(api_name, url, str(header) ,r.text, str(feilds),request_method=r.request.method)
    ...
    @classmethod
    def api_通过id查询巡检计划(cls, session: requests.Session, id: str,header:dict):
        api_name = 'api_通过id查询巡检计划'
    ...
    @classmethod
    def api_通过id删除巡检计划(cls, session: requests.Session, id: str,header:dict):
        api_name = 'api_通过id删除巡检计划'
        url = Api_Addr_WeRun.werun_apis.get(api_name)+id
        header.update(Browser_Param.headers.get('form').copy())
