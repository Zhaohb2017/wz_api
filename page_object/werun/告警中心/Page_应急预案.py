...
class Page应急预案(object):
    ...
    @classmethod
    def api_修改应急预案(cls, session: requests.Session, feilds: dict, header: dict):
        api_name = 'api_修改应急预案'
        url = Api_Addr_WeRun.werun_apis.get(api_name)
        header.update(Browser_Param.headers.get('json').copy())
    ...
    @classmethod
    def api_应急预案启用禁用(cls, session: requests.Session, feilds: dict, header: dict):
        api_name = 'api_应急预案启用禁用'
        url = Api_Addr_WeRun.werun_apis.get(api_name)
        header.update(Browser_Param.headers.get('json').copy())
        r = session.put(url, headers=header, json=feilds)
        Common_Base.logging_api_info(api_name, url, str(header), r.text, str(feilds))
    ...
    @classmethod
    def api_通过id查询应急预案(cls, session: requests.Session, id: str,header:dict):
        api_name = 'api_通过id查询应急预案'
