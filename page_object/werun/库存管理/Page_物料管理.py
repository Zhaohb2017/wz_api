...
class Page物料管理(object):

    @classmethod
    def api_物料入库(cls, session: requests.Session, feilds: dict,header:dict):
        api_name = 'api_物料入库'
    ...
    @classmethod
    def api_物料出库(cls, session: requests.Session, feilds: dict,header:dict):
        api_name = 'api_物料出库'
        url = Api_Addr_WeRun.werun_apis.get(api_name)
        header.update(Browser_Param.headers.get('json').copy())
        r = session.post(url, headers=header, json=feilds)
    ...
    @classmethod
    def api_物料分页查询(cls, session: requests.Session, feilds: dict,header:dict):
        api_name = 'api_物料分页查询'
        url = Api_Addr_WeRun.werun_apis.get(api_name)
        header.update(Browser_Param.headers.get('json').copy())
        r = session.post(url, headers=header, json=feilds)
    ...
    @classmethod
    def api_通过id查询物料(cls, session: requests.Session, id: str,header:dict):
        api_name = 'api_通过id查询物料'
