...
class Page班次管理(object):

    @classmethod
    def api_新增或更新班次以及时间安排(cls, session: requests.Session, feilds: dict,header:dict):
        api_name = 'api_新增或更新班次以及时间安排'
    ...
    @classmethod
    def api_查询班次数据范围信息(cls, session: requests.Session, feilds: dict,header:dict):
        api_name = 'api_查询班次数据范围信息'
        url = Api_Addr_WeRun.werun_apis.get(api_name)
        header.update(Browser_Param.headers.get('json').copy())
    ...
    @classmethod
    def api_分页查询班次信息(cls, session: requests.Session, feilds: dict,header:dict):
        api_name = 'api_分页查询班次信息'
        url = Api_Addr_WeRun.werun_apis.get(api_name)
        header.update(Browser_Param.headers.get('json').copy())
        r = session.post(url, headers=header, json=feilds)
        try:
    ...
    @classmethod
    def api_获取班次名称集合(cls, session: requests.Session, feilds: dict,header:dict):
        api_name = 'api_获取班次名称集合'
        url = Api_Addr_WeRun.werun_apis.get(api_name)
        header.update(Browser_Param.headers.get('json').copy())
        r = session.post(url, headers=header, json=feilds)
        Common_Base.logging_api_info(api_name, url, str(header) ,r.text, str(feilds),request_method=r.request.method)
    ...
    @classmethod
    def api_通过班次id查询班次时间安排(cls, session: requests.Session, id: str,header:dict):
        api_name = 'api_通过班次id查询班次时间安排'
    ...
    @classmethod
    def api_通过id删除班次(cls, session: requests.Session, id: str,header:dict):
        api_name = 'api_通过id删除班次'
        url = Api_Addr_WeRun.werun_apis.get(api_name)+id
        header.update(Browser_Param.headers.get('form').copy())
