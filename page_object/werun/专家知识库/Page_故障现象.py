...
class Page故障现象(object):
    ...
    @classmethod
    def api_修改故障现象(cls, session: requests.Session, feilds: dict,header:dict):
        api_name = 'api_修改故障现象'
        url = Api_Addr_WeRun.werun_apis.get(api_name)
        header.update(Browser_Param.headers.get('json').copy())
    ...
    @classmethod
    def api_查询故障描述集合(cls, session: requests.Session, feilds: dict,header:dict):
        api_name = 'api_查询故障描述集合'
        url = Api_Addr_WeRun.werun_apis.get(api_name)
        header.update(Browser_Param.headers.get('json').copy())
        r = session.post(url, headers=header, json=feilds)
        Common_Base.logging_api_info(api_name, url, str(header) ,r.text, str(feilds),request_method=r.request.method)
    ...
    @classmethod
    def api_故障现象分页查询(cls, session: requests.Session, feilds: dict,header:dict):
        api_name = 'api_故障现象分页查询'
        url = Api_Addr_WeRun.werun_apis.get(api_name)
        header.update(Browser_Param.headers.get('json').copy())
        r = session.post(url, headers=header, json=feilds)
    ...
    @classmethod
    def api_通过id查询故障现象(cls, session: requests.Session, id: str,header:dict):
        api_name = 'api_通过id查询故障现象'
    ...
    @classmethod
    def api_通过id删除故障现象(cls, session: requests.Session, id: str,header:dict):
        api_name = 'api_通过id删除故障现象'
        url = Api_Addr_WeRun.werun_apis.get(api_name)+id
        header.update(Browser_Param.headers.get('form').copy())
