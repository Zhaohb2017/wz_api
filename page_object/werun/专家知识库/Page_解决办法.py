...
class Page解决办法(object):
    ...
    @classmethod
    def api_修改解决方法(cls, session: requests.Session, feilds: dict,header:dict):
        api_name = 'api_修改解决方法'
        url = Api_Addr_WeRun.werun_apis.get(api_name)
        header.update(Browser_Param.headers.get('json').copy())
    ...
    @classmethod
    def api_分页查询解决办法(cls, session: requests.Session, feilds: dict,header:dict):
        api_name = 'api_分页查询解决办法'
        url = Api_Addr_WeRun.werun_apis.get(api_name)
        header.update(Browser_Param.headers.get('json').copy())
        r = session.post(url, headers=header, json=feilds)
    ...
    @classmethod
    def api_通过id查询解决方法(cls, session: requests.Session, id: str,header:dict):
        api_name = 'api_通过id查询解决方法'
    ...
    @classmethod
    def api_通过id删除解决方法(cls, session: requests.Session, id: str,header:dict):
        api_name = 'api_通过id删除解决方法'
        url = Api_Addr_WeRun.werun_apis.get(api_name)+id
        header.update(Browser_Param.headers.get('form').copy())
    ...
    @classmethod
    def api_查询故障现象集合(cls, session: requests.Session, feilds: str,header:dict):
