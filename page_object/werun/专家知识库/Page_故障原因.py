...
class Page故障原因(object):
    ...
    @classmethod
    def api_修改专家知识库_手册内容(cls, session: requests.Session, feilds: dict,header:dict):
        api_name = 'api_修改专家知识库_手册内容'
        url = Api_Addr_WeRun.werun_apis.get(api_name)
        header.update(Browser_Param.headers.get('json').copy())
    ...
    @classmethod
    def api_查询故障原因code集合(cls, session: requests.Session, feilds: dict,header:dict):
        api_name = 'api_查询故障原因code集合'
        url = Api_Addr_WeRun.werun_apis.get(api_name)
        header.update(Browser_Param.headers.get('json').copy())
        r = session.post(url, headers=header, json=feilds)
        Common_Base.logging_api_info(api_name, url, str(header) ,r.text, str(feilds),request_method=r.request.method)
    ...
    @classmethod
    def api_通过id查询故障原因(cls, session: requests.Session, id: str,header:dict):
        api_name = 'api_通过id查询故障原因'
    ...
    @classmethod
    def api_通过id删除故障原因(cls, session: requests.Session, id: str,header:dict):
        api_name = 'api_通过id删除故障原因'
        url = Api_Addr_WeRun.werun_apis.get(api_name)+id
        header.update(Browser_Param.headers.get('form').copy())
