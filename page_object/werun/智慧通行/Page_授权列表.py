...
class Page授权列表(object):

    @classmethod
    def api_新增授权(cls, session: requests.Session, feilds: dict,header:dict):
        api_name = 'api_新增授权'
    ...
    @classmethod
    def api_修改通行授权(cls, session: requests.Session, feilds: dict,header:dict):
        api_name = 'api_修改通行授权'
        url = Api_Addr_WeRun.werun_apis.get(api_name)
        header.update(Browser_Param.headers.get('json').copy())
    ...
    @classmethod
    def api_通过id查询授权(cls, session: requests.Session, id: str,header:dict):
        api_name = 'api_通过id查询授权'
    ...
    @classmethod
    def api_通过id删除授权(cls, session: requests.Session, id: str,header:dict):
        api_name = 'api_通过id删除授权'
        url = Api_Addr_WeRun.werun_apis.get(api_name)+id
        header.update(Browser_Param.headers.get('form').copy())
