...
class Page企业通行规则(object):
    ...
    @classmethod
    def api_修改企业通行规则(cls, session: requests.Session, feilds: dict, header: dict):
        api_name = 'api_修改企业通行规则'
        url = Api_Addr_WeRun.werun_apis.get(api_name)
        header.update(Browser_Param.headers.get('json').copy())
    ...
    @classmethod
    def api_通过id查询规则(cls, session: requests.Session, id: str, header: dict):
        api_name = 'api_通过id查询规则'
        url = Api_Addr_WeRun.werun_apis.get(api_name) + id
        header.update(Browser_Param.headers.get('json').copy())
