...
class Page能耗统计管理(object):

    @classmethod
    def api_新增统计分组(cls, session: requests.Session, feilds: dict,header:dict):
        api_name = 'api_新增统计分组'
    ...
    @classmethod
    def api_通过id查询分组(cls, session: requests.Session, feilds: dict,header:dict):
        api_name = 'api_通过id查询分组'
        url = Api_Addr_WeRun.werun_apis.get(api_name)
        header.update(Browser_Param.headers.get('json').copy())
        r = session.post(url, headers=header, json=feilds)
        Common_Base.logging_api_info(api_name, url, str(header) ,r.text, str(feilds),request_method=r.request.method)
    ...
    @classmethod
    def api_通过名称查询分组(cls, session: requests.Session, feilds: dict,header:dict):
        api_name = 'api_通过名称查询分组'
        url = Api_Addr_WeRun.werun_apis.get(api_name)
        header.update(Browser_Param.headers.get('json').copy())
        r = session.post(url, headers=header, json=feilds)
        Common_Base.logging_api_info(api_name, url, str(header) ,r.text, str(feilds),request_method=r.request.method)
    ...
    @classmethod
    def api_编辑分组(cls, session: requests.Session,feilds: dict,header:dict):
        api_name = 'api_编辑分组'
        url = Api_Addr_WeRun.werun_apis.get(api_name)
        header.update(Browser_Param.headers.get('json').copy())
    ...
    @classmethod
    def api_通过id删除分组(cls, session: requests.Session,id: str,header:dict):
        api_name = 'api_通过id删除分组'
    ...
    @classmethod
    def api_查询能耗统计图表(cls, session: requests.Session, feilds: dict, header:dict):
        api_name = 'api_查询能耗统计图表'
        url = Api_Addr_WeRun.werun_apis.get(api_name)
        header.update(Browser_Param.headers.get('form').copy())
        r = session.post(url, headers=header, json=feilds)
        Common_Base.logging_api_info(api_name, url, str(header) ,r.text, str(feilds),request_method=r.request.method)
